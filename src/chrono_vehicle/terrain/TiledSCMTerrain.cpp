...
// -----------------------------------------------------------------------------
// Implementation of the TiledSCMTerrain wrapper class
// -----------------------------------------------------------------------------

TiledSCMTerrain::TiledSCMTerrain(ChSystem* system, bool visualization_mesh) {
    m_loader = chrono_types::make_shared<TiledSCMLoader>(system, visualization_mesh);
    system->Add(m_loader);
}

// Get the initial terrain height below the specified location.
double TiledSCMTerrain::GetInitHeight(const ChVector<>& loc) const {
    return m_loader->GetInitHeight(loc);
}

// Get the initial terrain normal at the point below the specified location.
ChVector<> TiledSCMTerrain::GetInitNormal(const ChVector<>& loc) const {
    return m_loader->GetInitNormal(loc);
}

// Get the terrain height below the specified location.
double TiledSCMTerrain::GetHeight(const ChVector<>& loc) const {
    return m_loader->GetHeight(loc);
}

// Get the terrain normal at the point below the specified location.
ChVector<> TiledSCMTerrain::GetNormal(const ChVector<>& loc) const {
    return m_loader->GetNormal(loc);
}

// Return the terrain coefficient of friction at the specified location.
float TiledSCMTerrain::GetCoefficientFriction(const ChVector<>& loc) const {
    return m_friction_fun ? (*m_friction_fun)(loc) : 0.8f;
}

// Get SCM information at the node closest to the specified location.
TiledSCMTerrain::NodeInfo TiledSCMTerrain::GetNodeInfo(const ChVector<>& loc) const {
    return m_loader->GetNodeInfo(loc);
}

// Set the color of the visualization assets.
void TiledSCMTerrain::SetColor(const ChColor& color) {
...
void TiledSCMTerrain::SetTexture(const std::string tex_file, float scale_x, float scale_y) {
    if (m_loader->GetVisualModel()) {
...
void TiledSCMTerrain::SetCosimulationMode(bool val) {
    m_loader->m_cosim_mode = val;
}

// Set properties of the SCM soil model.
void TiledSCMTerrain::SetSoilParameters(
...
void TiledSCMTerrain::SetTestHeight(double offset) {
    m_loader->m_test_offset_up = offset;
}

double TiledSCMTerrain::GetTestHeight() const {
    return m_loader->m_test_offset_up;
}

// Set the color plot type.
void TiledSCMTerrain::SetPlotType(DataPlotType plot_type, double min_val, double max_val) {
...
// Enable moving patch.
void TiledSCMTerrain::AddMovingPatch(std::shared_ptr<ChBody> body,
                                const ChVector<>& OOBB_center,
                                const ChVector<>& OOBB_dims) {
    TiledSCMLoader::MovingPatchInfo pinfo;
    pinfo.m_body = body;
...
void TiledSCMTerrain::RegisterSoilParametersCallback(std::shared_ptr<SoilParametersCallback> cb) {
    m_loader->m_soil_fun = cb;
}

// Initialize the terrain as a flat grid.
void TiledSCMTerrain::Initialize(double sizeX, double sizeY, double delta) {
    m_loader->Initialize(sizeX, sizeY, delta);
}

// Initialize the terrain from a specified height map.
void TiledSCMTerrain::Initialize(const std::string& heightmap_file,
                            double sizeX,
                            double sizeY,
                            double hMin,
                            double hMax,
...
void TiledSCMTerrain::Initialize(const std::string& mesh_file, double delta) {
...
std::vector<TiledSCMTerrain::NodeLevel> TiledSCMTerrain::GetModifiedNodes(bool all_nodes) const {
    return m_loader->GetModifiedNodes(all_nodes);
}

// Modify the level of grid nodes from the given list.
void TiledSCMTerrain::SetModifiedNodes(const std::vector<NodeLevel>& nodes) {
    m_loader->SetModifiedNodes(nodes);
}

bool TiledSCMTerrain::GetContactForceBody(std::shared_ptr<ChBody> body, ChVector<>& force, ChVector<>& torque) const {
...
bool TiledSCMTerrain::GetContactForceNode(std::shared_ptr<fea::ChNodeFEAxyz> node, ChVector<>& force) const {
...
TiledSCMContactableData::TiledSCMContactableData(double area_ratio,
...
// Constructor.
TiledSCMLoader::TiledSCMLoader(ChSystem* system, bool visualization_mesh) : m_soil_fun(nullptr), m_base_height(-1000) {
    ...
    m_plot_type = TiledSCMTerrain::PLOT_NONE;
    m_plot_v_min = 0;
    m_plot_v_max = 0.2;
...
// Initialize the terrain as a flat grid
void TiledSCMLoader::Initialize(double sizeX, double sizeY, double delta) {
...
// Initialize the terrain from a specified height map.
void TiledSCMLoader::Initialize(const std::string& heightmap_file,
                           double sizeX,
                           double sizeY,
                           double hMin,
                           double hMax,
...
// Initialize the terrain from a specified OBJ mesh file.
bool TiledSCMLoader::calcBarycentricCoordinates(const ChVector<>& v1,
                                const ChVector<>& v2,
                                const ChVector<>& v3,
                                const ChVector<>& v,
                                double& a1,
...
void TiledSCMLoader::Initialize(const std::string& mesh_file, double delta) {
...
void TiledSCMLoader::Initialize(const geometry::ChTriangleMeshConnected& trimesh, double delta) {
...
void TiledSCMLoader::CreateVisualizationMesh(double sizeX, double sizeY) {
    int nvx = 2 * m_nx + 1;                     // number of grid vertices in X direction
    int nvy = 2 * m_ny + 1;                     // number of grid vertices in Y direction
...
bool TiledSCMLoader::CheckMeshBounds(const ChVector2<int>& loc) const {
    return loc.x() >= -m_nx && loc.x() <= m_nx && loc.y() >= -m_ny && loc.y() <= m_ny;
}

TiledSCMTerrain::NodeInfo TiledSCMLoader::GetNodeInfo(const ChVector<>& loc) const {
    TiledSCMTerrain::NodeInfo ni;
...
int TiledSCMLoader::GetMeshVertexIndex(const ChVector2<int>& loc) {
    assert(loc.x() >= -m_nx);
    assert(loc.x() <= +m_nx);
    assert(loc.y() >= -m_ny);
    assert(loc.y() <= +m_ny);
...
std::vector<int> TiledSCMLoader::GetMeshFaceIndices(const ChVector2<int>& loc) {
    int i = loc.x();
    int j = loc.y();
...
double TiledSCMLoader::GetInitHeight(const ChVector2<int>& loc) const {
    switch (m_type) {
        case PatchType::FLAT:
            return 0;
...
ChVector<> TiledSCMLoader::GetInitNormal(const ChVector2<int>& loc) const {
    switch (m_type) {
        case PatchType::HEIGHT_MAP:
        case PatchType::TRI_MESH: {
...
double TiledSCMLoader::GetHeight(const ChVector2<int>& loc) const {
    // First query the hash-map
...
ChVector<> TiledSCMLoader::GetNormal(const ChVector2<>& loc) const {
    switch (m_type) {
        case PatchType::HEIGHT_MAP:
        case PatchType::TRI_MESH: {
            // Average normals of 4 triangular faces incident to given grid node
...
// Get the initial terrain height below the specified location.
double TiledSCMLoader::GetInitHeight(const ChVector<>& loc) const {
    // Express location in the SCM frame
    ChVector<> loc_loc = m_plane.TransformPointParentToLocal(loc);
...
// Get the initial terrain normal at the point below the specified location.
ChVector<> TiledSCMLoader::GetInitNormal(const ChVector<>& loc) const {
    // Express location in the SCM frame
    ChVector<> loc_loc = m_plane.TransformPointParentToLocal(loc);
...
// Get the terrain height below the specified location.
double TiledSCMLoader::GetHeight(const ChVector<>& loc) const {
    // Express location in the SCM frame
    ChVector<> loc_loc = m_plane.TransformPointParentToLocal(loc);
...
// Get the terrain normal at the point below the specified location.
ChVector<> TiledSCMLoader::GetNormal(const ChVector<>& loc) const {
    // Express location in the SCM frame
    ChVector<> loc_loc = m_plane.TransformPointParentToLocal(loc);
...
void TiledSCMLoader::UpdateMovingPatch(MovingPatchInfo& p, const ChVector<>& Z) {
...
void TiledSCMLoader::UpdateFixedPatch(MovingPatchInfo& p) {
    ChVector2<> p_min(+std::numeric_limits<double>::max());
    ChVector2<> p_max(-std::numeric_limits<double>::max());
...
bool TiledSCMLoader::RayOBBtest(const MovingPatchInfo& p, const ChVector<>& from, const ChVector<>& Z) {
...
void TiledSCMLoader::ComputeInternalForces() {
    ...
    for (auto& h : hits) {
        ...
        //// TODO:  take into account "tread height" (add to TiledSCMContactableData)?

        if (auto cprops = contactable->GetUserData<vehicle::TiledSCMContactableData>()) {
...
void TiledSCMLoader::AddMaterialToNode(double amount, NodeRecord& nr) {
...
void TiledSCMLoader::RemoveMaterialFromNode(double amount, NodeRecord& nr) {
...
void TiledSCMLoader::UpdateMeshVertexCoordinates(const ChVector2<int> ij, int iv, const NodeRecord& nr) {
    auto& trimesh = *m_trimesh_shape->GetMesh();
    std::vector<ChVector<>>& vertices = trimesh.getCoordsVertices();
    std::vector<ChColor>& colors = trimesh.getCoordsColors();
...
void TiledSCMLoader::UpdateMeshVertexNormal(const ChVector2<int> ij, int iv) {
    auto& trimesh = *m_trimesh_shape->GetMesh();
    std::vector<ChVector<>>& vertices = trimesh.getCoordsVertices();
    std::vector<ChVector<>>& normals = trimesh.getCoordsNormals();
    std::vector<ChVector<int>>& idx_normals = trimesh.getIndicesNormals();
...
// Get the heights of modified grid nodes.
std::vector<TiledSCMTerrain::NodeLevel> TiledSCMLoader::GetModifiedNodes(bool all_nodes) const {
    std::vector<TiledSCMTerrain::NodeLevel> nodes;
    if (all_nodes) {
...
void TiledSCMLoader::SetModifiedNodes(const std::vector<TiledSCMTerrain::NodeLevel>& nodes) {
    for (const auto& n : nodes) {
        // Modify existing entry in grid map or insert new one
        m_grid_map[n.first] = TiledSCMLoader::NodeRecord(n.second, n.second, GetInitNormal(n.first));
    }
