#ifndef TILED_SCM_TERRAIN_H
#define TILED_SCM_TERRAIN_H
...
class CH_VEHICLE_API TiledSCMTerrain : public ChTerrain {
  public:
    ...
    TiledSCMTerrain(ChSystem* system,               ///< [in] containing multibody system
    ...
    std::shared_ptr<TiledSCMLoader> GetSCMLoader() const { return m_loader; }

    void SetBaseMeshLevel(double level);

  private:
    std::shared_ptr<TiledSCMLoader> m_loader;  ///< underlying load container for contact force generation
};

/// Parameters for soil-contactable interaction.
class CH_VEHICLE_API TiledSCMContactableData {
  public:
    TiledSCMContactableData(double area_ratio,     ///< area fraction with overriden parameters (in [0,1])
                       double Mohr_cohesion,  ///< cohesion for shear failure [Pa]
                       double Mohr_friction,  ///< friction angle for shear failure [degree]
                       double Janosi_shear    ///< shear parameter in Janosi-Hanamoto formula [m]
    );
  ...
  private:
    ...
    friend class TiledSCMLoader;
};

/// Underlying implementation of the Soil Contact Model.
class CH_VEHICLE_API TiledSCMLoader : public ChLoadContainer {
  public:
    TiledSCMLoader(ChSystem* system, bool visualization_mesh);
    ~TiledSCMLoader() {}
  ...
  private:
    ...
    // Calculate barycentric coordinates of the terrain from a specified OBJ mesh file.
    ...
    TiledSCMTerrain::NodeInfo GetNodeInfo(const ChVector<>& loc) const;
    ...
    /// If 'all_nodes = true', return modified nodes from the start of simulation.  Otherwise, return only the nodes
    /// modified over the last step.
    std::vector<TiledSCMTerrain::NodeLevel> GetModifiedNodes(bool all_nodes = false) const;

    // Modify the level of grid nodes from the given list.
    void SetModifiedNodes(const std::vector<TiledSCMTerrain::NodeLevel>& nodes);
    ...
    std::shared_ptr<TiledSCMTerrain::SoilParametersCallback> m_soil_fun;
    ...
    // Mesh coloring mode
    TiledSCMTerrain::DataPlotType m_plot_type;
    double m_plot_v_min;
    double m_plot_v_max;
